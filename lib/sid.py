...
class SID:
    ...
    def __init__(self, strSID):
        ...
        if strSID == "":
            return None
        (self.Revision, ) = unpack('B', unhexlify(strSID.strip()[:2]))
        (self.SecurityAuthority, ) = unpack('>I', unhexlify(strSID.strip()[8:16]))
        self.NumIDS = (len(strSID.strip()) / 8) - 2
        (self.RID, ) = unpack('>I',unhexlify(strSID.strip()[len(strSID.strip()) - 8:]))
        
        self.IDs = []
        for id in range(0, round(self.NumIDS - 1)):
            (tmp,) = unpack('I', unhexlify(strSID.strip()[(id + 2) * 8:(id + 2) * 8 + 8]))
            self.IDs.append(tmp)
